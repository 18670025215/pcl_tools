...
void printHelp(int, char** argv)
{
  ...
  print_info(")\n");
  print_info(" For organized datasets, an IntegralImageNormalEstimation "
             "approach will be used, with the RADIUS given value as SMOOTHING "
             "SIZE.\n");
  print_info("\nOptional arguments are:\n");
  print_info("                     -view_point vpx,vpy,vpz          = absolute "
             "view point in metres\n");
  print_info("                     -input_dir X  = batch process all PCD files "
             "found in input_dir\n");
  print_info("                     -output_dir X = save the processed files "
             "from input_dir in this directory\n");
}

bool loadCloud(const string& filename, pcl::PCLPointCloud2& cloud, Eigen::Vector4f& translation,
...
void saveCloud(const string& filename, const pcl::PCLPointCloud2& output, const Eigen::Vector4f& translation,
...
int main(int argc, char** argv)
{
  ...
  else
  {
    if (input_dir != "" && boost::filesystem::exists(input_dir))
    {
      ...
      for (boost::filesystem::directory_iterator itr(input_dir); itr != end_itr; ++itr)
      {
        ...
        if (!is_directory(itr->status()) &&
            boost::algorithm::to_upper_copy(boost::filesystem::extension(itr->path())) == ".PCD")
        {
    ...
    {
      PCL_ERROR("Batch processing mode enabled, but invalid input directory "
                "(%s) given!\n",
                input_dir.c_str());
      return (-1);
    }
